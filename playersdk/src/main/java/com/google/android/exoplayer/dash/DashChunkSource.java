...
public class DashChunkSource implements ChunkSource
{
     ...
     * @param formatEvaluator       Selects from the available formats.
     */
    public DashChunkSource(MediaPresentationDescription manifest,
    ...
    /* package */ DashChunkSource(ManifestFetcher<MediaPresentationDescription> manifestFetcher,
                                  MediaPresentationDescription initialManifest,
                                  int adaptationSetIndex,
                                  int[] representationIndices,
                                  DataSource dataSource,
                                  FormatEvaluator formatEvaluator,
                                  ...
                                  EventListener eventListener)
    {
        ...
        for (int i = 0; i < representations.length; i++)
        {
            ...
            representationHolders.put(formats[i].id,
                                      new RepresentationHolder(representations[i],
                                                               new ChunkExtractorWrapper(extractor)));
        }
  ...
  /* package */ TimeRange getSeekRange()
    ...
    public void continueBuffering(long playbackPositionUs)
    {
        ...
        {
            Representation[] newRepresentations = DashChunkSource.getFilteredRepresentations(
                    newManifest,
                    adaptationSetIndex,
                    representationIndices);
                                        ...
                                        long playbackPositionUs, ChunkOperationHolder out)
    {
        ...
        if (pendingInitializationUri != null || pendingIndexUri != null)
        {
            ...
            Chunk initializationChunk = newInitializationChunk(pendingInitializationUri,
                                                               pendingIndexUri,
        ...
        if (indexLastAvailableSegmentNum == DashSegmentIndex.INDEX_UNBOUNDED)
        {
            ...
            if (currentManifest.timeShiftBufferDepth != -1)
            {
                ...
                indexFirstAvailableSegmentNum = Math.max(indexFirstAvailableSegmentNum,
                                                         segmentIndex.getSegmentNum(
                                                                 liveEdgeTimestampUs - bufferDepthUs));
            }
    ...
    private void updateSeekRange(DashSegmentIndex segmentIndex, long nowUs)
    {
        ...
        if (currentManifest.dynamic)
        {
            ...
            latestSeekPosition = Math.max(earliestSeekPosition,
                                          liveEdgeTimestampUs - liveEdgeLatencyUs);
        }
    ...
    private Chunk newInitializationChunk(RangedUri initializationUri,
                                         RangedUri indexUri,
                                         Representation representation,
                                         ChunkExtractorWrapper extractor,
                                         DataSource dataSource,
                                         int trigger)
    {
        ...
        return new InitializationChunk(dataSource,
                                       dataSpec,
                                       trigger,
                                ...
                                int segmentNum, int trigger)
    {
        ...
        if (representation.format.mimeType.equals(MimeTypes.TEXT_VTT))
        {
            MediaFormat mediaFormat = MediaFormat.createTextFormat(MimeTypes.TEXT_VTT,
                                                                   MediaFormat.NO_VALUE,
                                                                   representation.format.language);
            return new SingleSampleMediaChunk(dataSource,
                                              dataSpec,
                                              Chunk.TRIGGER_INITIAL,
                                              representation.format,
                                              startTimeUs,
                                              endTimeUs,
                                              absoluteSegmentNum,
        ...
        {
            return new ContainerMediaChunk(dataSource,
                                           dataSpec,
                                           trigger,
                                           representation.format,
                                           startTimeUs,
    ...
    private static Representation[] getFilteredRepresentations(MediaPresentationDescription manifest,
                                                               ...
                                                               int[] representationIndices)
    {
        AdaptationSet adaptationSet = manifest.periods.get(0).adaptationSets.get(
                adaptationSetIndex);
    ...
    private static MediaPresentationDescription buildManifest(List<Representation> representations)
    {
        ...
        AdaptationSet adaptationSet = new AdaptationSet(0,
                                                        AdaptationSet.TYPE_UNKNOWN,
                                                        representations);
        Period period = new Period(null,
                                   firstRepresentation.periodStartMs,
                                   firstRepresentation.periodDurationMs,
