...
//                channelConfig = AudioFormat.CHANNEL_OUT_FRONT_RIGHT_OF_CENTER;
        ...
        minBufferSize = android.media.AudioTrack.getMinBufferSize(sampleRate,
                                                                  channelConfig,
                                                                  encoding);
            ...
            throws WriteException
    {
        ...
        if (temporaryBufferSize == 0)
        {
            ...
            else
            {
                ...
                {
                    Log.e(TAG,
                          "Discontinuity detected [expected " + expectedBufferStartTime + ", got "
                                  + bufferStartTime + "]");
                    startMediaTimeState = START_NEED_SYNC;
                }
        ...
        if (Util.SDK_INT < 21)
        {
            ...
            if (bytesToWrite > 0)
            {
                ...
                bytesWritten = audioTrack.write(temporaryBuffer,
    ...
    public void setChannel(int channel)
    {
        if (audioTrack != null)
        {
            try
            {
                if (channel == 3)
                {
                    audioTrack.setStereoVolume(0, 1.f);
                }
                else
                {
                    audioTrack.setStereoVolume(1.f, 0);
                }
            }
            catch (Exception e)
            {
                ...
                && systemClockUs - lastTimestampSampleTimeUs >= MIN_TIMESTAMP_SAMPLE_INTERVAL_US)
        {
            ...
            if (getLatencyMethod != null)
            {
                try
                {
                    ...
                    latencyUs = (Integer) getLatencyMethod.invoke(audioTrack,
                                                                  (Object[]) null) * 1000L
