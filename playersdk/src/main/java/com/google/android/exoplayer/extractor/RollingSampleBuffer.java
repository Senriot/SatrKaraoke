...
/* package */ final class RollingSampleBuffer
{
    ...
    private void readEncryptionData(SampleHolder sampleHolder, SampleExtrasHolder extrasHolder)
    {
        ...
        sampleHolder.cryptoInfo.set(subsampleCount,
                                    clearDataSizes,
                                    encryptedDataSizes,
                                    extrasHolder.encryptionKeyId,
                                    sampleHolder.cryptoInfo.iv,
    ...
    private void readData(long absolutePosition, byte[] target, int length)
    {
        ...
        while (bytesRead < length)
        {
            ...
            Allocation allocation = dataQueue.peek();
            System.arraycopy(allocation.data,
                             allocation.translateOffset(positionInAllocation),
                             target,
                             bytesRead,
                             toCopy);
            absolutePosition += toCopy;
    ...
    public void appendData(ParsableByteArray buffer, int length)
    {
        while (length > 0)
        {
            ...
            buffer.readBytes(lastAllocation.data,
                             lastAllocation.translateOffset(lastAllocationOffset),
                             thisAppendLength);
    ...
    private static final class InfoQueue
    {
                                              ...
                                              byte[] encryptionKey)
        {
            ...
            if (queueSize == capacity)
            {
                ...
                System.arraycopy(encryptionKeys,
                                 relativeReadIndex,
                                 newEncryptionKeys,
                                 0,
                                 beforeWrap);
